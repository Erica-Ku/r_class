...
# 단계 2: 오라클 데이터베이스 연결
conn <- dbConnect(drv,
                  "jdbc:oracle:thin:@//127.0.0.1:1521/xe", "c##koo", "1234")
...
# 단계 3: 레코드 삽입(insert)
query = "insert into test_table values('hong', '1234', 'kang', 45)"
...
query = "delete from test_table where name = 'kim'"
dbSendUpdate(conn, query)
...
recode <- read.csv("C:/Rwork/Part-II/recode.csv", header = TRUE, fileEncoding = "CP949", encoding = "UTF-8")
